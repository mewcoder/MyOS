...
- AI 数据问答一次只使用网站、GitHub、文章中的一个范围，不跨类型混合上下文；JSON 文件名不出现在界面中。
...
- 用户必须选择 `网站`、`GitHub` 或 `文章` 中的一个；AI 模式不提供“全部”范围，也不向用户展示 JSON 文件名。
- 从对应页面进入时默认选择当前类型；首页进入时默认选择 `文章`，但允许切换。
- 每次请求把选中的完整 JSON 作为上下文，不做候选召回、跨源拼接或 embeddings。
- JSON 发送前仅做稳定的空白压缩，不删除字段或记录。
- Prompt 明确要求只根据所选内容范围回答，并进行归纳、比较和解释，而不是只返回关键词过滤结果；资料不足时说“不足”，不得利用模型记忆补全。
- 请求前只显示人类可读的范围名称和记录数，不显示文件名或 JSON 字符数。
...
│                                                              │
│ AI 回答                                                      │
│ 与 Agent Skill 相关的仓库包括……                              │
│                                                              │
│ 基于“GitHub”中的 136 条收藏                                  │
└──────────────────────────────────────────────────────────────┘
```

- 原有关键词搜索界面和 AI 数据问答使用两个明确模式，关键词搜索仍为默认。
- 各页面都从顶栏“搜索 / AI”进入该界面，不增加独立的数据问答导航入口。
- “询问 AI”是显式提交，不对每个键入字符调用 API。
- 流式答案显示当前内容范围；用户可随时停止。
...
Desktop 将文章 AI 放在目录下方，与目录共用左侧 sticky 功能栏；tablet/mobile 则放在文章元信息之后、正文之前。打开后，当前文章 Markdown 是唯一正文上下文。
...
- Desktop AI 区域位于目录下方；tablet/mobile 位于元信息之后、正文之前。两种布局都可折叠、清空对话、重新生成和停止。
